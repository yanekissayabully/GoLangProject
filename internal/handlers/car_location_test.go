...
// mockCarRepo implements a minimal in-memory car repo for testing
type mockCarRepo struct {
	store map[uuid.UUID]*models.Car
}
...
func TestUpdateCarLocation_NotOwner(t *testing.T) {
	...
	requestBody := models.UpdateCarLocationRequest{
		Latitude:  ptrFloat(40.123),
		Longitude: ptrFloat(-73.456),
	}
	...
	ctx := context.WithValue(req.Context(), httputil.UserIDKey, requestUserID)
	req = req.WithContext(ctx)
...
func TestUpdateCarLocation_MissingLatLng(t *testing.T) {
	requestBody := map[string]string{"area": "Brooklyn"}
	bodyBytes, _ := json.Marshal(requestBody)
...
func TestUpdateCarLocationRequest_Encoding(t *testing.T) {
	...
	testLng := -73.456
	testArea := "Brooklyn"
	testStreet := "162 Crescent St"
	testBlock := "4"
	testZip := "NY 11208"

	originalReq := models.UpdateCarLocationRequest{
		Latitude:  &testLat,
		Longitude: &testLng,
		Area:      &testArea,
		Street:    &testStreet,
	...
	var decodedReq models.UpdateCarLocationRequest
	if err := json.Unmarshal(data, &decodedReq); err != nil {
		t.Fatalf("failed to unmarshal: %v", err)
	}

	if decodedReq.Latitude == nil || *decodedReq.Latitude != testLat {
		t.Errorf("latitude mismatch: expected %f, got %v", testLat, decodedReq.Latitude)
	}
	if decodedReq.Longitude == nil || *decodedReq.Longitude != testLng {
		t.Errorf("longitude mismatch: expected %f, got %v", testLng, decodedReq.Longitude)
	}
	if decodedReq.Area == nil || *decodedReq.Area != testArea {
		t.Errorf("area mismatch: expected %s, got %v", testArea, decodedReq.Area)
	}
	if decodedReq.Street == nil || *decodedReq.Street != testStreet {
		t.Errorf("street mismatch: expected %s, got %v", testStreet, decodedReq.Street)
	}
	if decodedReq.Block == nil || *decodedReq.Block != testBlock {
		t.Errorf("block mismatch: expected %s, got %v", testBlock, decodedReq.Block)
	}
	if decodedReq.Zip == nil || *decodedReq.Zip != testZip {
		t.Errorf("zip mismatch: expected %s, got %v", testZip, decodedReq.Zip)
	}
}
...
func TestCarLocationResponse_Fields(t *testing.T) {
	testLat := 40.123
	testLng := -73.456
	response := models.CarLocationResponse{
		Address:      "162 Crescent St",
		Neighborhood: "Downtown",
		Latitude:     &testLat,
		Longitude:    &testLng,
	...
	for _, fieldName := range []string{"address", "neighborhood", "latitude", "longitude", "area", "street", "block", "zip"} {
